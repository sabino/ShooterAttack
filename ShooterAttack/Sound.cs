...
namespace ShooterAttack
{
    static class Sound
    {
        ...
        private static SoundEffect[] moving;
        public static SoundEffect Moving { get { return moving[rand.Next(moving.Length)]; } }
        ...
        {
            Music = content.Load<Song>("Sound/Song");

            explosions = Enumerable.Range(1, 8).Select(x => content.Load<SoundEffect>("Sound/explosion-0" + x)).ToArray();
            shots = Enumerable.Range(1, 1).Select(x => content.Load<SoundEffect>("Sound/shoot-0" + x)).ToArray();
            spawns = Enumerable.Range(1, 1).Select(x => content.Load<SoundEffect>("Sound/spawn-0" + x)).ToArray();
            moving = Enumerable.Range(1, 1).Select(x => content.Load<SoundEffect>("Sound/moving-0" + x)).ToArray();
        }
    }
}
