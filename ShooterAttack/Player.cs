...
namespace ShooterAttack
{
    static class Player
    {
        ...
        private static float minSoundVolume = 0.1f;
        ...
        private static float _remainingDelay;

        public static Vector2 PathingNodePosition
        {
            get
            {
                return Map.GetSquareAtPixel(BaseSprite.WorldCenter);
            }
        }
        ...
        private static Vector2 checkTileObstacles(float elapsedTime, Vector2 moveAngle)
        {
            ...
            if (moveAngle.X != 0)
            {
                for (int x = horizLeftPixel; x < horizRightPixel; x++)
                {
                    ...
                    {
                        if (Map.IsWallTileByPixel(new Vector2(x, newHorizontalLocation.Y + y)))
                        {
                            moveAngle.X = 0;
                            break;
                        }
            ...
            if (moveAngle.Y != 0)
            {
                for (int y = vertTopPixel; y < vertBottomPixel; y++)
                {
                    ...
                    {
                        if (Map.IsWallTileByPixel(new Vector2(newVerticalLocation.X + x, y)))
                        {
                            moveAngle.Y = 0;
                            break;
                        }
        ...
        public static void Update(GameTime gameTime)
        {
           ...
           {
               Sound.Moving.Play(0.2f, new Random().NextFloat(-0.2f, 0.2f), 0);
               _remainingDelay = _delay;
           }
           else
           {
               Sound.Moving.Play(0.4f, new Random().NextFloat(-0.2f, 0.2f), 0);
           }*/
